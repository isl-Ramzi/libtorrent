...
using tracker_request_flags_t = flags::bitfield_flag<std::uint8_t, struct tracker_request_flags_tag>;
	...
	protected:
		...
		// true whenever m_req's outcome (success, error, or a deliberate
		// fire-and-forget dispatch) has not yet been reported to m_requester.
		// fail_impl() clears it when it reports an error; subclasses clear it
		// wherever else they report m_req's outcome (or deliberately choose
		// not to, e.g. a write-only dispatch), and set it again when m_req is
		// replaced with a new, as yet unreported request. A subclass's close()
		// checks it to report a request that would otherwise be torn down
		// with no outcome ever communicated.
