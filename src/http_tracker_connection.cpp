...
namespace libtorrent::aux {
	...
	void http_tracker_connection::next_request()
	{
		...
		// on the same connection: http_connection's drain state (m_draining)
		// is never reset when switching back to a normal, response-reading
		// dispatch, which would race a fresh read against the still-running
		// drain loop. prune_followers() is relied on to keep this from
		// happening, by stripping non-stop followers from every pooled
		// connection during shutdown before any of this could race.
