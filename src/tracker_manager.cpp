...
namespace libtorrent::aux {
		...
		, std::string const msg, seconds32 const interval, seconds32 const min_interval)
	{
		...
		// pending by the time it runs: close() may have already reported it
		// (e.g. tracker_manager::abort_all_requests() closing this
		// connection directly while this call was still queued). Checking
		// here, rather than in fail(), is what makes the two mutually exclusive.
		if (m_req_pending)
		{
			m_req_pending = false;
